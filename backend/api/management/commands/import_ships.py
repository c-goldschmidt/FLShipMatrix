...
from api.models import Ship, ShipModelLOD, Texture
...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        with ZipFile(filename, 'r') as zf:
            ...
            self._import_ships(zf, ship_data['ships'])
            self._import_textures(zf, ship_data['texture_ids'])

    @classmethod
    def _import_ships(cls, zf, data):
        for import_id in data:
            cls._import_ship(zf, data[import_id], import_id)
            data[import_id] = None


    @classmethod
    def _import_ship(cls, zf, data, import_id):
        full_data = {}
    ...
    def _import_textures(cls, zf, texture_ids):
        for tex_id in texture_ids:
            try:
                tex_data = zf.read('{}.tex'.format(tex_id))
            except KeyError:
                print('could not find a texture for {}'.format(tex_id))
                continue
