...
uniform vec4 u_DiffuseMix;
uniform vec2 u_OpacityMix;
...
#ifdef HAS_BASECOLORMAP
    vec4 baseColor = texture2D(uTextureSampler, v_UV) * u_BaseColorFactor;

    baseColor = mix(baseColor, baseColor * vec4(u_DiffuseMix.rgb, 1.0), u_DiffuseMix.a);
    baseColor = mix(baseColor, vec4(0, 0, 0, 0), u_OpacityMix.y > 0.0 ? 1.0 - u_OpacityMix.x : 0.0);
#else
